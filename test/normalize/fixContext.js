...
export function addsMissingContext() {
  it("automatically adds missing @context", () => {
    ...
    assert.strictEqual(
      Object.keys(person.worksFor[0]).includes("@context"),
      false,
    );
    assert.strictEqual(
      Object.keys(person.alumniOf[0]).includes("@context"),
      false,
    );
    assert.strictEqual(
      Object.keys(person.lifeEvent[0]).includes("@context"),
      false,
    );
  });
}
...
export function removesRedundantContexts() {
  ...
  it("automatically removes redundant @context from", () => {
    test("worksFor", () => {
      ...
      assert.strictEqual(
        Object.keys(worksFor[0].worksFor).includes("@context"),
        false,
      );
    });
    ...
    test("alumniOf", () => {
      ...
      assert.strictEqual(
        Object.keys(alumniOf[0].alumniOf).includes("@context"),
        false,
      );
    });
    ...
    test("hasCertification", () => {
      ...
      assert.strictEqual(
        Object.keys(hasCertification).includes("@context"),
        false,
      );
    });
    ...
    test("hasCredential", () => {
      ...
      assert.strictEqual(
        Object.keys(hasCredential).includes("@context"),
        false,
      );
    });
