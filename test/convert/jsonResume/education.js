  ...
  describe("education           ->  alumniOf", () => {
    ...
    test("@type", () => {
      for (var i = 0; i < person.alumniOf.length; i++) {
        ...
        assert.strictEqual(
          person.alumniOf[i].alumniOf["@type"],
    ...
    test("institution       ->  alumniOf.name", () => {
      for (var i = 0; i < jsonresume.education.length; i++) {
        assert.strictEqual(
          person.alumniOf[i].alumniOf.name,
          jsonresume.education[i].institution,
        );
      }
    });
    test("area              ->  description", () => {
      for (var i = 0; i < jsonresume.education.length; i++) {
        assert.strictEqual(
          person.alumniOf[i].description,
          jsonresume.education[i].area,
        );
      }
    });
    test("startDate         ->  startDate", () => {
      for (var i = 0; i < jsonresume.education.length; i++) {
        if (
          jsonresume.education[i].startDate ||
          jsonresume.education[i].endDate
        ) {
          assert.strictEqual(
            person.alumniOf[i].startDate,
            jsonresume.education[i].startDate,
          );
        } else {
    ...
    test("location          ->  worksFor.location", () => {
      for (var i = 0; i < jsonresume.education.length; i++) {
        assert.strictEqual(
          person.alumniOf[i].alumniOf.location,
          jsonresume.education[i].location,
        );
      }
    });
