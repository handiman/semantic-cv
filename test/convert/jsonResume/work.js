...
export default () =>
  describe("work                ->  worksFor", () => {
    ...
    test("summary           ->  description", () => {
      for (var i = 0; i < jsonresume.work.length; i++) {
        if (jsonresume.work[i].summary) {
          assert.notStrictEqual(worksFor[i].description.indexOf(jsonresume.work[i].summary), -1);
        }
      }
    });
    ...
    test("highlights        ->  description", () => {
      for (var i = 0; i < jsonresume.work.length; i++) {
        if (jsonresume.work[i].highlights) {
          assert.notStrictEqual(
            worksFor[i].description.indexOf(jsonresume.work[i].highlights.join("\n")),
            -1
          );
        }
      }
    ...
    test("startDate         ->  startDate", () => {
      for (var i = 0; i < jsonresume.work.length; i++) {
        if (
          ...
          assert.strictEqual(worksFor[i].startDate, jsonresume.work[i].startDate);
        } else {
    ...
    test("location          ->  worksFor.location", () => {
      for (var i = 0; i < jsonresume.work.length; i++) {
        assert.strictEqual(worksFor[i].worksFor.location, jsonresume.work[i].location);
      }
    });
