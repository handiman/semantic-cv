...
export default () =>
  describe("awards              ->  lifeEvent", () => {
    ...
    test("title             ->  name", () => {
      for (var i = 0; i < jsonresume.awards.length; i++) {
        assert.strictEqual(person.lifeEvent[i].name, jsonresume.awards[i].title);
      }
    });

    test("date              ->  startDate", () => {
      for (var i = 0; i < jsonresume.awards.length; i++) {
        assert.strictEqual(person.lifeEvent[i].startDate, jsonresume.awards[i].date);
      }
    });

    test("summary           ->  description", () => {
      for (var i = 0; i < jsonresume.awards.length; i++) {
        assert.strictEqual(person.lifeEvent[i].description, jsonresume.awards[i].summary);
      }
    });
