  ...
  describe("certificates        ->  hasCertification", () => {
    ...
    test("name              ->  name", () => {
      for (var i = 0; i < jsonresume.certificates.length; i++) {
        assert.strictEqual(person.hasCertification[i].name, jsonresume.certificates[i].name);
      }
    });

    test("issuer            ->  issuedBy", () => {
      for (var i = 0; i < jsonresume.certificates.length; i++) {
        assert.strictEqual(person.hasCertification[i].issuedBy, jsonresume.certificates[i].issuer);
      }
    });

    test("date              ->  validFrom", () => {
      for (var i = 0; i < jsonresume.certificates.length; i++) {
        assert.strictEqual(person.hasCertification[i].validFrom, jsonresume.certificates[i].date);
      }
    });
