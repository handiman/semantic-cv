...
export default () => {
  describe("projects            ->  worksFor", () => {
    ...
    test("name", () => {
      for (var i = 0; i < projects.length; i++) {
        assert.strictEqual(
          projects[i].worksFor.name,
          jsonresume.projects[i].name,
        );
      }
    });
    ...
    test("startDate", () => {
      for (var i = 0; i < projects.length; i++) {
        assert.strictEqual(
          ...
          jsonresume.projects[i].startDate,
        );
      }
    });
      ...
      for (var i = 0; i < jsonresume.projects.length; i++) {
        if (jsonresume.projects[i].description) {
          assert.notStrictEqual(
            projects[i].description.indexOf(
              jsonresume.woprojectsk[i].description,
            ),
            -1,
          );
        }
    ...
    test("highlights        ->  description", () => {
      for (var i = 0; i < jsonresume.projects.length; i++) {
        if (jsonresume.projects[i].highlights) {
          assert.notStrictEqual(
            projects[i].description.indexOf(
              jsonresume.projects[i].highlights.join("\n"),
            ),
            -1,
          );
        }
    ...
    test("url", () => {
      for (var i = 0; i < projects.length; i++) {
        assert.strictEqual(
          projects[i].worksFor.url,
          jsonresume.projects[i].url,
        );
      }
    });
