...
const sameAs = (writer: WritableStreamDefaultWriter) => (person: any) => {
  const { sameAs } = person;
  (sameAs ?? []).map((url: string) =>
    writer.write(`${initCaps(toHost(url))}: ${url}\n`),
  );
  return person;
};
...
const projects = (writer: WritableStreamDefaultWriter) => (person: any) => {
  const { worksFor } = person;
  const filtered = (worksFor ?? []).filter(
    (item: any) => item.worksFor["@type"] === "Project",
  );
...
const knowsLanguage =
  (writer: WritableStreamDefaultWriter) => (person: any) => {
    const { knowsLanguage } = person;
    if (knowsLanguage && knowsLanguage.length > 0) {
      writer.write(`\nLanguages\n`);
      writer.write(
        knowsLanguage.map((language: string) => `- ${language}`).join("\n"),
      );
    }
    return person;
  };
